...
- **Implemented:** MiniLM dependency and local model cache; reusable backend relatedness provider;
  ...
  adaptive lifecycle ownership; backend recovery; a priority-aware visible iOS Model Task queue; automatic
...
- **Not wired yet:** midpoint bridge persistence and explicit reorganize/merge operations remain.
  ...
  multimodal, and multi-turn testing.
...
It is a concise, app-generated public approach based on the user's question. Specialized summaries
must apply only to questions they actually fit (for example, act/intention/circumstances is for
judging an action, not every question containing “moral”). The fallback includes a shortened focus
from the question so it remains visibly relevant rather than displaying generic filler.
...
- **Language model:** Gemma 4 E2B loaded through MLX-VLM with the Aquinas LoRA adapter applied to
  ...
  checkpoint's vision tower for image understanding.
...
Return the visible answer and a separate list of key terms that the UI should underline.
...
The backend validates that each `display_text` actually occurs in `response`. The client may turn
validated terms into its current `aq://` links, but the model should not be trusted to produce
correct application URLs itself. Key terms prioritize concepts and subjects that are
intellectually foundational or prerequisite to understanding the answer. Interesting discovery
terms may also be highlighted when they offer a meaningful rabbit hole; novelty alone is not
enough, and there is no highlighting quota.
...
`LiteRTAquinasModel` runs ordinary conversation locally, emits a concise public approach summary
at generation start, and asks for an answer whose length is proportional to the question rather
than enforcing a paragraph quota. Its deterministic sampler matches the original MLX runtime so
4-bit quantization noise is not amplified by a broad high-temperature sample. It extracts foundational and interesting key terms from the
completed response and returns direct-definition Insight metadata. It also adapts compaction, contextual definitions,
